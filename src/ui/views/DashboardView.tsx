...
export const DashboardView: React.FC = () => {
  ...
  if (measuredResources.length > 0) {
    ...
    if (minPercent < 15) {
      ...
      capacityRiskSubtext = t('dashboard.capacityHighSubtext');
    } else if (minPercent < 35) {
      capacityRiskType = 'MODERATE';
      capacityRiskLabel = t('dashboard.capacityModerate');
      capacityRiskSubtext = t('dashboard.capacityModerateSubtext');
    } else {
      capacityRiskType = 'LOW';
      capacityRiskLabel = t('dashboard.capacityLow');
      capacityRiskSubtext = t('dashboard.capacityLowSubtext');
    }
  }

  // Real project status label
  const projectStatusLabel = activeProject
    ? activeProject.status === 'READY'
      ? t('header.statusReady')
    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto overflow-y-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-surface-card border border-surface-border rounded-xl p-5 shadow space-y-2">
          ...
          <div className="text-xs text-slate-400">{t('dashboard.completedDesc')}</div>
        </div>

        <div className="bg-surface-card border border-surface-border rounded-xl p-5 shadow space-y-2">
          <div className="flex items-center justify-between text-slate-400 text-xs font-mono">
            <span>{t('dashboard.blockers')}</span>
            <AlertTriangle className="w-4 h-4 text-forge-rose" />
          </div>
          <div className="text-2xl font-bold text-white font-mono">{blockedTasks}</div>
          <div className="text-xs text-slate-400">{t('dashboard.blockersDesc')}</div>
        </div>

        <div className="bg-surface-card border border-surface-border rounded-xl p-5 shadow space-y-2">
          <div className="flex items-center justify-between text-slate-400 text-xs font-mono">
            <span>{t('dashboard.capacityRisk')}</span>
            <Zap className="w-4 h-4 text-forge-amber" />
          </div>
          <div className={`text-2xl font-bold font-mono ${capacityRiskType === 'LOW' ? 'text-emerald-400' : capacityRiskType === 'HIGH' ? 'text-rose-400' : 'text-slate-400'}`}>
            {capacityRiskLabel}
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          >
            <span>{t('dashboard.viewAllAgents')}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-surface-card border border-surface-border rounded-xl p-6 shadow-lg space-y-4">
          <h3 className="text-sm font-semibold text-white font-mono uppercase tracking-wider flex items-center space-x-2">
            ...
            <span>{t('dashboard.ownerAttentionTitle')}</span>
          </h3>

          {blockedTasks === 0 ? (
            <div className="p-4 bg-emerald-950/20 border border-emerald-800/30 rounded-lg text-xs text-emerald-300 space-y-1">
              <div className="font-semibold">{t('dashboard.allSystemsNormalTitle')}</div>
              <p className="text-slate-400">
                {t('dashboard.allSystemsNormalDesc')}
              </p>
            </div>
          ) : (
            <div className="p-4 bg-rose-950/20 border border-rose-800/30 rounded-lg text-xs text-rose-300 space-y-2">
              <div className="font-semibold">{t('dashboard.humanAttentionTitle', { count: blockedTasks.toString() })}</div>
              <button
              ...
              >
                {t('dashboard.inspectBlockedTasks')}
              </button>
            </div>
          )}

          <div className="p-4 bg-surface rounded-lg border border-surface-border space-y-2 text-xs">
            <div className="text-slate-400 font-mono">{t('dashboard.manualRelayTitle')}:</div>
            <p className="text-slate-300">
              {t('dashboard.manualRelayDesc')}
            </p>
            <button
            ...
            >
              <span>{t('dashboard.openManualBridge')}</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-surface-card border border-surface-border rounded-xl p-6 shadow-lg space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-white font-mono uppercase tracking-wider flex items-center space-x-2">
              ...
              <span>{t('dashboard.liveStreamTitle')}</span>
            </h3>
            <button
