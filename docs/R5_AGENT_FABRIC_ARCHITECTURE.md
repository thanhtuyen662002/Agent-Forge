...
- **Watermark Pair Coherence & Auth Validation**: The application watermark `(last_applied_action_account_order, last_applied_action_authorization_id)` is a logical pair where valid persisted states are strictly both absent (NULL) or both present and valid (order > 0 and authorization_id is a non-empty, non-whitespace string). Migration 15 has no DB-level pair or non-empty constraint, and the mapper preserves raw non-null strings so the runtime fail-closed guard can inspect them. Trim is used for validation only and does not alter identity comparison; authorization identity comparisons remain exact strings. Any malformed, partial, empty, or whitespace-only watermark pair fails closed with `status = 'REJECTED'`, error code `WATERMARK_PAIR_INTEGRITY_MISMATCH`, and zero database writes.
