...
| **FI-09** | **Handoff Interruption** | Process crash or severing during mid-task handoff dispatch. | `HandoffTransferService` / `ExecutionRecoveryScanner`. | Predecessor process ceases. Task state remains fail-closed in `HANDOFF_REQUIRED`. The durable handoff record truthfully reflects the exact interruption checkpoint in `HandoffTransferStatusEnum`: (1) If severed after predecessor ownership relinquishment before successor initialization: `status = 'RELINQUISHED'`; (2) If severed after successor preparation (attempt $N+1$ created and bound to snapshot `successor_context_snapshot_id`) before routing/dispatch: `status = 'SUCCESSOR_PREPARED'`; (3) If severed after routing authorization before dispatch acceptance: `status = 'AUTHORIZED'`. Task never advances to `CODING` until successor dispatch accepted. | Task safely paused in `HANDOFF_REQUIRED`; predecessor execution halted; zero split-brain execution. | `tasks.state == 'HANDOFF_REQUIRED'`; `handoff_transfers.status` in (`'RELINQUISHED'`, `'SUCCESSOR_PREPARED'`, or `'AUTHORIZED'`). | Idempotent resumption via `HandoffTransferService.prepareHandoffSuccessor()` (if `RELINQUISHED`) or `routeHandoffSuccessor()` / `resumeHandoffSuccessor()` (if `SUCCESSOR_PREPARED`), or explicit cancellation via `cancelHandoff()`. | `BLOCKED — deterministic injection harness not implemented` |
