...
- **Strict Separation of Concerns & Service Decoupling**:
  ...
  2. **Task State Cancellation**:
     ...
     - For recovery-fenced submissions, human owner cancellation executes via `CoderSubmissionAdjudicationService.acknowledgeRecoveryFenced(params)`:
       ...
       - **Behavior on `CANCEL`**: Adjudication status advances to `VERIFICATION_FAILED` (`completed_at = nowIso`), records resolution columns (`resolution_action = 'CANCEL'`, `resolution_evidence_json`, `resolution_evidence_hash`, `resolver_id`), emits `CODER_SUBMISSION_RECOVERY_CANCELLED`, and if the live task is in `VALIDATING`, `CODING`, or `BLOCKED`, applies `TaskStateMachine.transition(liveTask.state, 'CANCEL')` $\rightarrow$ `CANCELLED`.
       - Note: Does not release worker slot leases (already released/fenced during startup scan) and does not terminate OS processes.
