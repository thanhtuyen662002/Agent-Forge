...
| **FI-05** | **Agent Process Termination** | Coder process killed via OS signals during execution. | Active coder execution in `ProcessRunner`. | Non-zero exit code or process termination recorded in `process_runs`. Adapter catches termination and returns `status: 'FAILED'`. Task transitions via `TaskStateMachine` to `NEEDS_HUMAN` (or `CODING` if attempts remain). Slot lease release requires explicit lease management invocation. | User notified of agent exit; task transitions to fail-closed state (`NEEDS_HUMAN`). | `process_runs.exit_code != 0`, `process_runs.status == 'FAILED'`. | Slot lease released via `WorkerSlotLeaseService.releaseLease()`; fresh attempt allocated if attempts remain. | `SAFE FOR REHEARSAL ONLY` |
| **FI-06** | **Cancellation During Execution** | Task actively executing in `CODING` state. | `ProcessRunner.cancel(executionId)` and `TaskService.transitionTaskState()`. | `ProcessRunner.cancel()` terminates child process tree and returns `ProcessTerminationTruth` (`PROCESS_TREE_TERMINATED_PROVEN`, `TERMINATION_UNRESOLVED`, or `NOT_APPLICABLE`). Process termination is strictly decoupled from task state and lease release: `ProcessRunner.cancel()` does not update task state or release leases. Task moves to `CANCELLED` only via explicit call path `TaskService.transitionTaskState(..., trigger: 'CANCEL')`. Slot lease release requires explicit call path `WorkerSlotLeaseService.releaseLease(leaseId)`. Lease release is unsafe and prohibited if termination returned `TERMINATION_UNRESOLVED`. | Child process terminated. If orchestrated call path executed: task moves to `CANCELLED`, lease released. If `TERMINATION_UNRESOLVED`: alert operator of unconfirmed process state. | `ProcessRunner` return value; orchestrated path: `tasks.state == 'CANCELLED'`, `account_leases.released_at IS NOT NULL`. | If process proven dead: clean up worktree directory; mark worker slot `IDLE`. If `TERMINATION_UNRESOLVED`: conduct manual host process audit before lease release. | `SAFE FOR REHEARSAL & LIVE` *(process cancellation verified; task/lease cancellation requires orchestrator call path)* |
...
- **Strict Separation of Concerns**:
  - `ProcessRunner.cancel()` operates exclusively on the OS process tree. It does **not** update task state and does **not** release slot or coder_submission_workspace_leases.
  - Transitioning task state to `CANCELLED` requires an explicit orchestrator call path (e.g. `TaskService.transitionTaskState(..., 'CANCEL')` or `CoderSubmissionAdjudicationService.acknowledgeRecoveryFencedSubmission(..., 'CANCEL')`), which updates `tasks.state` to `CANCELLED`.
  - Releasing worker slot leases requires calling `WorkerSlotLeaseService.releaseLease(leaseId)`, which sets `account_leases.released_at` and restores the slot to `IDLE`.
  - **Fencing Precondition**: Slot lease release and worktree cleanup are strictly prohibited if process termination returned `TERMINATION_UNRESOLVED`. Reallocating resources while a process may still be running risks concurrent file corruption and split-brain execution.
...
| **Process Termination vs Task/Lease Lifecycle** | `ProcessRunner.cancel()` (`src/core/services/ProcessRunner.ts:1033`), `TaskService.transitionTaskState()` (`src/core/services/TaskService.ts:195`), `WorkerSlotLeaseService.releaseLease()` | `tests/processRunner.test.ts`, `tests/taskStateMachine.test.ts`; returns typed `ProcessTerminationTruth` | `ProcessRunner.cancel()` only terminates OS process; does not update `tasks.state` or release leases. Task `CANCELLED` and lease release require explicit orchestrator call paths. If `TERMINATION_UNRESOLVED`, lease release is unsafe. |
...
| **Context Manifest Hashing & Continuity** | `ContextBuilderService.buildContextSnapshot()` (`src/core/services/ContextBuilderService.ts`), Migration 9 | `tests/r5DurableMemoryContext.test.ts`; `context_manifests.manifest_hash` | Context snapshot hash and task memories are preserved identically across handoff transitions. |
