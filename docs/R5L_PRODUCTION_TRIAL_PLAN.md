...
| **FI-13** | **Application Restart Between Stages** | Terminate and relaunch AgentForge between Step 11 and 13. | Startup recovery in `CoderSubmissionAdjudicationRecoveryScanner.scanAndReconcile()`. | SQLite state preserved; restart scanner reconciles pending records. | Upon relaunch, task resumes in exact durable state (`REVIEW_READY`). | Recovery scanner startup audit log. | Resumes without re-running verification or mutating hashes. | `SAFE FOR REHEARSAL & LIVE` |
...
- In a systemic failure, `EmergencyStopService.triggerEmergencyStop(reason)` terminates all active child processes via `ProcessRunner.terminateAllProcesses()` (truthfully awaiting termination before recording events), transitions all `RUNNING` projects to `PAUSED`, transitions all active in-progress tasks (`DISPATCHED`, `CODING`, `VALIDATING`, `REVIEWING`) to `PAUSED` (recording `pausedFromState` and `TASK_PAUSED` events), and emits canonical `EMERGENCY_STOP` events per affected project.
