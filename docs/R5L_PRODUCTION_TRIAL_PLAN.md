...
sequenceDiagram
    ...
    Reviewer->>MCP_Rev: 14. Call agentforge_get_review_package / review://package (read-only, zero DB writes)
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **FI-01** | **Account Exhaustion Failover** | Inject rate-limit error into active provider account. | `RoleAwareRoutingService` / `FailoverNextRoutePolicyService`. | Account health moves to `RATE_LIMITED` or `QUOTA_EXHAUSTED` (via `ProviderHealthObservationService`). Router selects next candidate. | Coder dispatched to secondary account; task progresses without user intervention. | `provider_health_observations` row with `account_order`; routing event logged. | Cooldown backoff expires; health restored to `AVAILABLE`. | `SAFE FOR REHEARSAL ONLY` *(mock provider)* |
| **FI-02** | **Provider Outage Failover** | Simulate provider process failure / auth rejection. | `ProviderDispatchService`. | Provider marked `AUTH_ERROR` or `UNHEALTHY`. Route failover triggers fallback provider. | Transparent failover to secondary provider; audit event recorded. | `events` table: `PROVIDER_FAILOVER_DISPATCHED`. | Provider health check passes upon network restoration. | `SAFE FOR REHEARSAL ONLY` *(mock provider)* |
...
| **FI-05** | **Agent Process Termination** | Coder process killed via OS signals during execution. | Active coder execution in `ProcessRunner`. | Non-zero exit code recorded; attempt marked `FAILED`. Task transitions via `TaskStateMachine`. | User notified of agent exit; task transitions to `NEEDS_HUMAN` or retried if attempts remain. | `process_runs.exit_code != 0`. | Slot lease released; fresh attempt allocated if attempts remain. | `SAFE FOR REHEARSAL ONLY` |
| **FI-06** | **Cancellation During Execution** | Task actively in `CODING` state. | User triggers cancellation via `ProcessRunner.cancel()`. | Process runner terminates process tree cleanly; task state moves to `CANCELLED`. | Child process terminated cleanly; lease released. | `tasks.state == 'CANCELLED'`; process run completed. | Clean up worktree directory; mark worker slot `IDLE`. | `SAFE FOR REHEARSAL & LIVE` |
...
| **FI-09** | **Handoff Interruption** | Process severed during mid-task handoff dispatch. | Handoff context transition in `HandoffTransferService`. | Predecessor marked `RELINQUISHED`; successor not yet `DISPATCHED`. | Task pauses safely in `HANDOFF_REQUIRED`. | `handoff_transfers` row in `PREPARED` status. | Re-dispatch successor or return to human supervisor. | `BLOCKED — deterministic injection harness not implemented` |
...
| **FI-11** | **Reviewer Projection Hash Drift** | Stored projection hash tampered with in database. | Reviewer read call (`agentforge_get_review_package` or `review://package`). | MCP server rejects read with `PROJECTION_HASH_MISMATCH`. | Reviewer client receives clear error; zero context leaked. | Reviewer error response with code `PROJECTION_HASH_MISMATCH`. | Invalidate tampered session; re-issue from authentic adjudication. | `BLOCKED — deterministic injection harness not implemented` |
| **FI-12** | **Reviewer Token Expiry / Revocation** | Issue session with minimum TTL (`duration_seconds = 60`) or revoke via `ReviewerAuthorityService.revokeReviewerSession()`. | Authenticated read via tool `agentforge_get_review_package` and resource `review://package`. | Read rejected with `TOKEN_EXPIRED` (zero DB writes) or `TOKEN_REVOKED`. Transport handshake succeeds. | Reviewer client blocked from reading projection data. | Expired: read fails zero-write. Revoked: `mcp_reviewer_sessions.revoked_at` timestamp. | Issue fresh authorized reviewer session via owner bridge. | `SAFE FOR REHEARSAL & LIVE` |
| **FI-13** | **Application Restart Between Stages** | Terminate and relaunch AgentForge between Step 11 and 13. | Startup recovery in `CoderSubmissionAdjudicationRecoveryScanner`. | SQLite state preserved; restart scanner reconciles pending records. | Upon relaunch, task resumes in exact durable state (`REVIEW_READY`). | Recovery scanner startup audit log. | Resumes without re-running verification or mutating hashes. | `SAFE FOR REHEARSAL & LIVE` |
...
- In a systemic failure, `EmergencyStopService.stopAll()` executes global termination across all running projects.
...
| **Active Workspace Lease Unique Index** | SQLite Index | `src/core/database/migrations.ts` | `idx_coder_submission_workspace_leases_active` ON `coder_submission_workspace_leases(worktree_identity_hash) WHERE released_at IS NULL` (Migration 23) |
...
| **Handoff Transfers Table** | SQLite Schema | `src/core/database/migrations.ts` | `CREATE TABLE IF NOT EXISTS handoff_transfers` (Migration 17) |
...
| **Emergency Stop Service** | Core Service | `src/core/services/EmergencyStopService.ts` | `class EmergencyStopService`, `stopAll()` |
...
| **Adjudication Recovery Scanner** | Core Service | `src/core/services/CoderSubmissionAdjudicationRecoveryScanner.ts` | `class CoderSubmissionAdjudicationRecoveryScanner`, `scanAndReconcileAdjudications()` |
...
| **Reviewer Resource Template** | MCP Protocol Constant | `src/mcp/reviewerProtocol.ts` | `review://package` (`REVIEWER_URI_TEMPLATE`) |
