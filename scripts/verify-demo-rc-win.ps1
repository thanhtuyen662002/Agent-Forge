...
# Assertion B: Database migration count == 20
...
if (-not (Test-Path $migrationsPath)) {
  ...
  if ($migrationCount -ne 20) {
    $failures.Add("B_MIGRATION_COUNT_INVALID: Expected exactly 20 migrations, found $migrationCount")
  }
}
$receiptLines.Add("MIGRATION_COUNT=$migrationCount")
Write-Host "[B] Database Migration Count ($migrationCount): $(if ($migrationCount -eq 20) {'PASS'} else {'FAIL'})"
