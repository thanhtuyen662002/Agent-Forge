...
# Assertion B: Database migration count == 17
...
if (-not (Test-Path $migrationsPath)) {
  ...
  if ($migrationCount -ne 17) {
    $failures.Add("B_MIGRATION_COUNT_INVALID: Expected exactly 17 migrations, found $migrationCount")
  }
}
$receiptLines.Add("MIGRATION_COUNT=$migrationCount")
Write-Host "[B] Database Migration Count ($migrationCount): $(if ($migrationCount -eq 17) {'PASS'} else {'FAIL'})"
