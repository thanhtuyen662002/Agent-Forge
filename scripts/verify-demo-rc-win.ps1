...
# Assertion B: Database migration count == 15
...
if (-not (Test-Path $migrationsPath)) {
  ...
  if ($migrationCount -ne 15) {
    $failures.Add("B_MIGRATION_COUNT_INVALID: Expected exactly 15 migrations, found $migrationCount")
  }
}
$receiptLines.Add("MIGRATION_COUNT=$migrationCount")
Write-Host "[B] Database Migration Count ($migrationCount): $(if ($migrationCount -eq 15) {'PASS'} else {'FAIL'})"
