...
# Assertion B: Database migration count == 8
...
if (-not (Test-Path $migrationsPath)) {
  ...
  if ($migrationCount -ne 8) {
    $failures.Add("B_MIGRATION_COUNT_INVALID: Expected exactly 8 migrations, found $migrationCount")
  }
}
$receiptLines.Add("MIGRATION_COUNT=$migrationCount")
Write-Host "[B] Database Migration Count ($migrationCount): $(if ($migrationCount -eq 8) {'PASS'} else {'FAIL'})"
