...
# Assertion B: Database migration count == 9
...
if (-not (Test-Path $migrationsPath)) {
  ...
  if ($migrationCount -ne 9) {
    $failures.Add("B_MIGRATION_COUNT_INVALID: Expected exactly 9 migrations, found $migrationCount")
  }
}
$receiptLines.Add("MIGRATION_COUNT=$migrationCount")
Write-Host "[B] Database Migration Count ($migrationCount): $(if ($migrationCount -eq 9) {'PASS'} else {'FAIL'})"
