...
(async () => {
  try {
    ...
    console.log("Starting R5J4 Packaged Coder Submission Stdio Verification...");
    ...
    subDb.prepare("UPDATE execution_authorizations SET status = 'DISPATCHED', dispatched_at = ?, execution_id = ?, repository_head_sha = ? WHERE id = ?").run(subNow, 'exec-pkg-1', realHeadSha, authorizationId);
