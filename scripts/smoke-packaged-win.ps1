...
const workerSlotId = 'slot-pkg-' + crypto.randomUUID();
db.prepare(`
...
const accountLeaseId = 'lease-pkg-' + crypto.randomUUID();
db.prepare(`
...
(async () => {
  try {
    ...
    if (!fs.existsSync(adjServicePath) || !fs.existsSync(verServicePath) || !fs.existsSync(pkgGenPath) || !fs.existsSync(artStorePath)) {
      throw new Error("Packaged adjudication services missing in app.asar");
    }
...
'@
  ...
  if ($mcpProc.ExitCode -ne 0 -or -not ($mcpStdout -match "R5J3_MCP_BRIDGE_PROOF=PASS") -or -not ($mcpStdout -match "R5J4_MCP_SUBMISSION_PROOF=PASS") -or -not ($mcpStdout -match "R5J5_OWNER_ADJUDICATION_PROOF=PASS") -or -not ($mcpStdout -match "R5J6_SUBMISSION_OBSERVABILITY_PROOF=PASS")) {
  ...
  if (Test-Path $tempAppData) {
    Remove-PathWithBoundedRetry -Path $tempAppData
  }
